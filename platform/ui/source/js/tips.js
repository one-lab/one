	 ...
	 $.fn.extend({
		tips: function(opts){
			var defaults = {
				...
				//follow    : false,      //是否跟随
				winTipSite: "leftTop"          //windowTip坐标位置:center居中\leftTop左上角\rightTop右上角\leftBottom左下角\rightBottom右下角,自定义坐标:left数值\top数值
			}
			...
			function tipLocate(origin,theTip,e){
				...
				if(defaults.pointer == false){
					...
					if(defaults.tipPostion == "bottom"){
						...
						Math.abs(top = origin.offset().top - theTip.height() - 9);
