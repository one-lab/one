...
public interface RequestPerMinuteRepository extends PagingAndSortingRepository<RequestPerMinute, String> {
	...
	@SQL("select sum(m.request_number) / (count(1)*60) from ge_monitor_request_per_minute m WHERE m.application_id=?1 and m.record_time >= ?2 AND m.record_time <= ?3")
	BigDecimal staRequestPerMinutes(String applicationId, Date startDate, Date endDate);
}
