...
public interface EumUrlAvaRepository extends PagingAndSortingRepository<EumUrlAva, String> {
     ...
     @SQL("select count(1) as count ,interval from GE_MONITOR_EUM_URL_AVA where eum_url_id=?1 and state =?2 and RECORD_TIME>?3 GROUP BY INTERVAL")
     public List<AvailableCalculate.AvailableCountsGroupByInterval> countsGroupByIntervalAndRecordTimeAfterParamDate(String eumUrlId,String state,Date paramDate);
