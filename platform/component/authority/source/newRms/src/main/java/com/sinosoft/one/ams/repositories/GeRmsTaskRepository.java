...
public interface GeRmsTaskRepository extends PagingAndSortingRepository<Task, String>{
	...
	//根据功能ID查询功能对象
	@SQL("select taskId from GE_RMS_TASK where taskId in (?1) and isValidate='1' and sysFlag in('RMS',?2)")
	List<String>findTaskByTaskIds(List<String> taskIds,String sysFlag);
