...
function  checkLength2(field,min,max)
{
 ...
 for(var i=0;i<value.length;i++)
  ...
  if(count<minlength)
  {
   errorMessage(desc + "输入的内容不够！\n" + desc + "的最小长度为" + minlength + "个英文字符！\n请重新输入！");
    //field.focus();
   // field.select();
    return false;
  }
...
* @作者 Liujun
* @时间 2008-10-11
* @功能 将一个ID内的input元素剪切到另一个ID内，并保存原始值
*/
...
function getCardKey(){
  ...
  {
      errorMessage("数字证书获取异常，可能是没有安装控件，请联系IT人员解决："+e.message);
  }
	return info;
}	
/**
...
function getLocalIPAddress()
{
    ...
    {
        errorMessage("本地IP获取异常，可能是没有安装控件，请联系IT人员解决："+e.message);
    }
