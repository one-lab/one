...
<html>
	<head>
		...
		<script type="text/javascript" src="${ctx}/prpall/report/reinsAudit/ReinsAuditInput.js"></script>
		<script type="text/javascript" src="${ctx}/prpall/report/js/ReinsAuditApply.js"></script>
		<script type="text/javascript">
			//查询再保审核信息
			function findReinsAuditInfo(){
				var findUrl=contextRootPath + "/prpall/findReinsAuditInfo.do";
				$("#fmReinsAudit").attr("action",findUrl).submit();
			}
			...
			function findAuditIdea(){
				...
				var findAuditIdeaUrl = contextRootPath + "/prpall/findAuditIdea.do?lcReinsAudit.id.repNo=" + $('input:radio[name="radioRepNo"]:checked').val();
				$("#fmReinsAudit").attr("action",findAuditIdeaUrl).submit();
			}
		</script>
	</head>
	
	<body>
		<form id="fmReinsAudit" name="fm" method="post" onkeypress="KeyDown()">
			<table  class="common" cellpadding="3" cellspacing="0">
				<tr>
					<td class="formtitle" colspan="6"><img src="${ctx}/images/bgformtitle.gif" style="cursor:hand;">呈保申请信息</td>
				</tr>
				<tr>
					...
					<td class="right"><input id="name" name="lcReport.name" class="common" type="text" value="${lcReport.name }"></td>					
				</tr>				
				<tr> 
					<td class="left">呈报机构：</td>
					<td class="right">
					<input type="hidden" id="upperComCode" value='2110000000' />
					<input id="manageCom" name="lcReport.manageCom" class="codecode" type="text" onchange="clickable()" ondblclick="queryCode('manageCom','comName','PrpDcompany','comCode:manageCom|upperComCode:upperComCode')" style="width:20%" value='<s:if test="#request.manageCom==null" ><s:property value="#session.prpDcompany.comCode"/></s:if><s:else><s:property value="#request.manageCom"/></s:else>'><input id="comName" name="comName" class="common" type="text" onchange="clickable()" style="width:68%" value='<s:property value="#session.prpDcompany.comCName"/>' ><img src="${ctx}/images/bgMarkMustInput.jpg" ></td>
					<td class="left">呈报日期：</td>
					<td class="right">
						<input id="repApplyDate" name="lcReport.repApplyDate" id="repApplyDate" class="common" type="text" onchange="clickable()" style="width: 73%" value='<s:if test="#request.repApplyDate==null" ><%= new SimpleDateFormat("yyyy-MM-dd").format(new Date())%></s:if><s:else><s:date name="#request.repApplyDate" format="yyyy-MM-dd"/></s:else>'>
			...
			<table>
				<tr>
					<td colspan="6">
						<input type="button" class="cssbutton" name="queryButton" value="查  询" onClick="findReinsAuditInfo()">
					</td>
				</tr>
			</table>
			...
			<table>
				<tr>
					<td colspan="6">
						<input type="button" class="cssbutton" name="queryButton" value="进入再保审核" onClick="findAuditIdea()">
					</td>
				</tr>
			</table>
		</form>
