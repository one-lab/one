...
/**
 * User: Morgan Date: 12-8-15 Time: 下午4:24
 */
public abstract class OneAbstractRepositoryConfigDefinitionParser<S extends GlobalRepositoryConfigInformation<T>, T extends SingleRepositoryConfigInformation<S>>
  ...
  private static final Log
      LOG =
      LogFactory.getLog(OneAbstractRepositoryConfigDefinitionParser.class);

  private static final String
      REPOSITORY_INTERFACE_POST_PROCESSOR =
      "com.sinosoft.one.data.jpa.repository.support.OneRepositoryInterfaceAwareBeanPostProcessor";
  ...
  /**
   * Executes repository auto configuration by scanning the provided base package for repository
   * interfaces.
   */
  private void doAutoConfiguration(S config, ParserContext parser) {
    ...
    for (String repositoryInterface : repositoryInterfaces) {
      registerGenericRepositoryFactoryBean(parser, config
          .getAutoconfigRepositoryInformation(repositoryInterface));
    }
  }
  ...
  /**
   * Proceeds manual configuration by traversing the context's {@link
   * SingleRepositoryConfigInformation}s.
   */
  ...
  /**
   * Registers a generic repository factory bean for a bean with the given name and the provided
  ...
  private void registerGenericRepositoryFactoryBean(ParserContext parser, T context) {
    ...
    try {
      ...
      builder.addPropertyValue("queryLookupStrategyKey",
                               getOneQueryLookupStrategyKey(context.getSource()));

      builder.addPropertyValue("namedQueries",
                               new NamedQueriesBeanDefinitionParser(
                                   context.getNamedQueriesLocation())
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("Registering repository: " + context.getBeanId() + " - Interface: " + context
            .getInterfaceName()
      ...
      BeanComponentDefinition
          definition =
          new BeanComponentDefinition(beanDefinition, context.getBeanId());
  ...
  private OneQueryLookupStrategy.Key getOneQueryLookupStrategyKey(Element element) {
    ...
    return StringUtils.hasText(createFinderQueries) ? OneQueryLookupStrategy.Key
   ...
   */
  protected void postProcessBeanDefinition(T context, BeanDefinitionBuilder builder,
  ...
  /**
   * Registers a possibly available custom repository implementation on the repository bean. Tries
   * to find an already registered bean to reference or tries to detect a custom implementation
   * itself.
   *
  ...
  private String registerCustomImplementation(T config, ParserContext parser, Object source) {
    ...
    if (config.autodetectCustomImplementation()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Registering custom repository implementation: " + config.getImplementationBeanName()
            + " "
   ...
   *
   * @return the {@code AbstractBeanDefinition} of the custom implementation or {@literal null} if
   * none found
   */
  ...
  /**
   * Callback to register additional bean definitions for a {@literal repositories} root node. This
   * usually includes beans you have to set up once independently of the number of repositories to
   * be created. Will be called before any repositories bean definitions have been registered.
   */
  protected void registerBeansForRoot(BeanDefinitionRegistry registry, Object source) {

    AbstractBeanDefinition
        definition =
        BeanDefinitionBuilder.rootBeanDefinition(REPOSITORY_INTERFACE_POST_PROCESSOR)
            .getBeanDefinition();
  ...
  /**
   * Custom {@link ClassPathScanningCandidateComponentProvider} scanning for interfaces extending
   * the given base interface. Skips interfaces annotated with {@link
  ...
  static class RepositoryComponentProvider extends ClassPathScanningCandidateComponentProvider {
    ...
    protected boolean isCandidateComponent(AnnotatedBeanDefinition beanDefinition) {
      ...
      boolean
          ...
          !isGenericRepositoryInterface(beanDefinition.getBeanClassName());
    ...
    /**
     * {@link org.springframework.core.type.filter.TypeFilter} that only matches interfaces. Thus
     * setting this up makes only sense providing an interface type as {@code targetType}.
     *
     * @author Oliver Gierke
     */
    ...
    /**
     * A simple filter which matches classes with a given annotation, checking inherited annotations
     * as well.
     *
    ...
    private static class AnnotationTypeFilter extends AbstractTypeHierarchyTraversingFilter {
      ...
      /**
       * Create a new AnnotationTypeFilter for the given annotation type. This filter will also
       * match meta-annotations. To disable the meta-annotation matching, use the constructor that
       * accepts a ' <code>considerMetaAnnotations</code>' argument. The filter will not match
       * interfaces.
       *
      ...
      protected boolean matchSelf(MetadataReader metadataReader) {
        ...
        return metadata.hasAnnotation(this.annotationType.getName())
               || (this.considerMetaAnnotations && metadata
