...
(function ($) {
	 $.fn.extend({
		tabs: function(opts){
			...
			$('ul li:first',tab).addClass('tabs_select');
			$('div:first',tab).addClass('tabs_show');
			$('ul',tab).addClass('tabs_titles').wrap('<div class="tab_scroll"></div>');
			...
			$('ul li',tab).hover(function(){
				$(this).addClass('li_hover');
			},function(){
				$(this).removeClass('li_hover');
			});
			$('ul li:not(".disabled")',tab).live('click',function(){
				tabCut($(this))
			});
			
			if(defaults.closeTab == true){
				$('ul li:not(".disabled")',tab).wrapInner('<b class="has_close"></b>').prepend('<span class="close"></span>');
				$('ul li.disabled',tab).wrapInner('<b></b>');
				var tabClose = $('ul li span',tab);
				var txtTab = $('ul li b',tab);
				...
				$('ul li',tab).wrapInner('<b href="javascript:void(0)"></b>');
			};
			...
			function closeTba(theTba){
				...
				if($('ul.tabs_titles li',tab).size()==1){
					$('ul li',tab).addClass('tabs_select');
					$('ul li span',tab).remove();
					$('ul li b',tab).removeClass('has_close');
			...
			jQuery.extend({
				addTabFn : function(tab,hasClose,nodeText,nodeId,nodeBody,nodeUrl){
					...
					if(hasClose==true){
						addTabUl.append('<li><span class="close"></span><b class="has_close">' + nodeText + '</b></li>');
					}else{
						addTabUl.append('<li><b>' + nodeText + '</b></li>');
					};
					tab.append('<div class="tabs_cont">' + nodeBody + '</div>');
					oldWidth = oldWidth + $('ul.tabs_titles li:last',tab).width() + 12 + 1;
					$('ul.tabs_titles li:last',tab).click();
