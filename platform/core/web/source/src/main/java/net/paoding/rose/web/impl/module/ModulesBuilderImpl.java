...
import net.paoding.rose.util.ResourceLoaderUtil;
...
public class ModulesBuilderImpl implements ModulesBuilder {
	...
	//start by kylin 定义全局拦截器和未注册的拦截器优先级的取值范围（rose自带的拦截器除外）
     			...
     			for (int j = 0, length = interceptors.size() ; j < length; j++) {
     				...
     				ControllerInterceptor temp = InterceptorDelegate
     						.getMostInnerInterceptor(interceptorDelegate
     								.getInterceptor());
     				if (temp.getClass().getName().startsWith("net.paoding.rose.web") ||
     						temp.getClass().getName().startsWith("net.paoding.rose.controllers")) {
     					continue;
     				}
