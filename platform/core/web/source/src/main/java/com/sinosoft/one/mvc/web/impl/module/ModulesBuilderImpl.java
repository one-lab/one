...
import com.sinosoft.one.mvc.util.ResourceLoaderUtil;
...
import com.sinosoft.one.mvc.web.OncePerRequestInterceptorDelegate;
import com.sinosoft.one.mvc.web.ParamValidator;
import com.sinosoft.one.mvc.web.advancedinterceptor.Ordered;
import com.sinosoft.one.mvc.web.annotation.Ignored;
import com.sinosoft.one.mvc.web.annotation.Interceptor;
import com.sinosoft.one.mvc.web.annotation.NotForSubModules;
...
public class ModulesBuilderImpl implements ModulesBuilder {
	...
	//start by kylin 定义全局拦截器和未注册的拦截器优先级的取值范围（mvc自带的拦截器除外）
	private final int GLOBAL_INTERCEPTOR_CONFIG_START_PRIORITY = Integer.MAX_VALUE-10000;
	private final int INTERCEPTOR_NOT_CONFIG_START_PRIORITY = -1;
	//end by kylin
    ...
    public List<Module> build(List<ModuleResource> moduleResources,
        ...
        /*
         * ModuleResource 封装一个web模块资源的相关信息
         * 循环modelResources得到对应的model
         *  
         */
            ...
            for (Iterator<InterceptorDelegate> iter = interceptors.iterator(); iter.hasNext();) {
                ...
                if (!most.getClass().getName().startsWith("com.sinosoft.one.mvc.web")) {
                    ...
                    if (moduleResource.getInterceptedDeny() != null) {
                        if (MvcStringUtil.matches(moduleResource.getInterceptedDeny(), interceptor
                                .getName())) {
                            iter.remove();
                            if (logger.isDebugEnabled()) {
                                logger.debug("module '" + module.getMappingPath()
                                        + "': remove interceptor by mvc.properties: "
                    ...
                    if (moduleResource.getInterceptedAllow() != null) {
                        if (!MvcStringUtil.matches(moduleResource.getInterceptedAllow(),
                                interceptor.getName())) {
                            iter.remove();
                            if (logger.isDebugEnabled()) {
                                logger.debug("module '" + module.getMappingPath()
    ...
    private List<InterceptorDelegate> findInterceptors(XmlWebApplicationContext context) throws FileNotFoundException, ParserConfigurationException, SAXException, IOException {
        ...
        for (String beanName : interceptorNames) {
            ...
            if (beanName.startsWith(AUTO_BEAN_NAME_PREFIX)) {
                interceporName = StringUtils.removeEnd(StringUtils.uncapitalize(userClass
                        .getSimpleName()), MvcConstants.INTERCEPTOR_SUFFIX);
            } else {
            ...
            if (interceporName.startsWith(mvc)
                    && (interceporName.length() == mvc.length() || Character
                            .isUpperCase(interceporName.charAt(mvc.length())))
                ...
                throw new IllegalArgumentException("illegal interceptor name '" + interceporName
                        ...
                        + ": don't starts with 'mvc', it's reserved");
            }
     		...
     		for (String configInterName : rList) {
     			for (int j = 0, length = interceptors.size() ; j < length; j++) {
     				...
     				ControllerInterceptor temp = InterceptorDelegate
     						.getMostInnerInterceptor(interceptorDelegate
     								.getInterceptor());
     					...
     					if(configInterName.trim().equals(temp.getClass().getName())){
     						if (temp instanceof ControllerInterceptorAdapter) {
     							((ControllerInterceptorAdapter) temp)
     									.setPriority(this.GLOBAL_INTERCEPTOR_CONFIG_START_PRIORITY - tempGlobalPriority++);
     							continue;
     						} else{
     							if (temp instanceof ControllerInterceptorAdapter) {
     								((ControllerInterceptorAdapter) temp)
     				...
     				if(logger.isDebugEnabled()){
     					logger.debug("Interceptor's Priority:"+temp.getClass().getName()+":"+((ControllerInterceptorAdapter) temp).getPriority());
     				}
     			}
     		}
        }
