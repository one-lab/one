...
public class MappingNode implements Comparable<MappingNode> {
    ...
    public ArrayList<MatchResult> match(RequestPath requestPath) {
        ...
        // 给当前判断结点判断的path
        String remaining = requestPath.getMvcPath();
        ...
        while (true) {
            ...
            if (last == null) {
                ...
                if (curNode.sibling == null) {
                    if (debugEnabled) {
                        logger.debug("['" + requestPath.getMvcPath() + "'] not matched");
                    }
                    return null;
                }
            ...
            if (debugEnabled) {
                logger.debug("['" + requestPath.getMvcPath() + "'] matched(" //
            ...
            if (remaining.length() == 0) {
                if (debugEnabled) {
                    logger.debug("['" + requestPath.getMvcPath() + "'] matched over.");
                }
                return matchResults;
            }
            //
