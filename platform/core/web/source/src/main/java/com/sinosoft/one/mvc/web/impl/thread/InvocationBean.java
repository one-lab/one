...
import com.sinosoft.one.mvc.web.var.ModelImpl;
...
public final class InvocationBean implements Invocation {
    ...
    private <T extends Engine> T getEngine(Class<? extends Engine> engineClass) {
        for (LinkedEngine engine : mvc.getEngines()) {
    ...
    public Model getModel() {
        ...
        synchronized (this) {
            ModelImpl model = (ModelImpl) getRequest().getAttribute("$$one-mvc.model");
            if (model == null || model.getInvocation() != this) {
                ...
                getRequest().setAttribute("$$one-mvc.model", model);
            }
            this.model = model;
        }
        return this.model;
    ...
    public Flash getFlash(boolean create) {
        ...
        Flash flash = (Flash) getRequest().getAttribute("$$one-mvc.flash");
        if (flash == null && create) {
            flash = new FlashImpl(this);
            getRequest().setAttribute("$$one-mvc.flash", flash);
        }
    ...
    public String getResourceId() {
        ...
        for (MatchResult matchResult : mvc.getMatchResults()) {
