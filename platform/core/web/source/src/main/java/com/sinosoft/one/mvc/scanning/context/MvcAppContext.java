 ...
 * 
 */
public class MvcAppContext extends AbstractXmlApplicationContext {
    ...
    public MvcAppContext(String scope, boolean refresh) {
        this(new LoadScope(scope, "applicationContext"), refresh);
    }

    public MvcAppContext(LoadScope scope, boolean refresh) {
        this.scopeValues = scope.getScope("applicationContext");
        logger.info("create a MvcAppContext, with scope='" + scope + "'");
        if (refresh) {
            refresh();
        }
    }
    ...
    protected Resource[] getConfigResourcesThrowsIOException() throws IOException {
        return MvcResources.findContextResources(this.scopeValues).toArray(new Resource[0]);
    }
    ...
    public MvcAppContext getApplicationContext() {
        return this;
    }
