 ...
 * 
 */
public class Mvc implements EngineChain {

    protected static final Log logger = LogFactory.getLog(Mvc.class);
    ...
    public Mvc(List<Module> modules, MappingNode mappingTree, HttpServletRequest httpRequest,
    ...
    /**
     * 启动mvc逻辑，对请求进行匹配判断，如果匹配未能成功返回false； <br>
    ...
    private boolean innerStart() throws Throwable {
        ...
        if (matchResults == null) {
            // not mvc uri
            if (debugEnabled) {
                logger.debug("not mvc uri: '" + this.path.getUri() + "'");
            }
            return false;
        }
        ...
        if (leafEngineGroup.size() == 0) {
            // not mvc uri
            if (debugEnabled) {
                logger.debug("not mvc uri, not exits leaf engines for it: '" + this.path.getUri()
                        + "'");
            }
            return false;
