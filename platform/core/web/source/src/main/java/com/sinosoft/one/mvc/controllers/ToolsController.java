...
import com.sinosoft.one.mvc.web.impl.mapping.MatchResult;
import com.sinosoft.one.mvc.web.impl.module.ControllerRef;
...
public class ToolsController {
    ...
    public String tools(Invocation inv) {
        ...
        s += "<a href=\"" + prefix + "tree\">/mvc-info/tree</a></div>";

        s += "<div style='margin-left:20px'>";
        s += "<a href=\"" + prefix + "modules\">/mvc-info/modules</a></div>";

        s += "<div style='margin-left:20px'>";
        s += "<a href=\"" + prefix + "resources\">/mvc-info/resources</a></div>";

        s += "<div style='margin-left:20px'>";
        s += "<a href=\"" + prefix + "method\">/mvc-info/method</a></div>";

        s += "<div style='margin-left:20px'>";
        s += "<a href=\"" + prefix + "startupInfo\">/mvc-info/startupInfo</a></div>";
        s += "<p>";
        return Utils.wrap(s);
    }
    ...
    public String tree(Mvc mvc) throws Exception {
        MappingNode root = mvc.getMappingTree();
    ...
    public Object method(Invocation pinv) {
        ...
        if (testUri == null || testUri.length() == 0) {
            return "@e.g. <a href='/mvc-info/method?get=/mvc-info/tree'>/mvc-info/method?get=/mvc-info/tree</a>";
        }
        ...
        MappingNode tree = inv.getMvc().getMappingTree();
        ...
        if (matchResults == null) {
            // not mvc uri
            return ("@404: <br>not mvc uri: '" + testUri + "'");
        }
        ...
        if (leafEngine == null) {
            if (leafEngineGroup.size() == 0) {
                // not mvc uri
                return ("@404: <br>not mvc uri, not exits leaf engines for it: '" + testUri + "'");
    ...
    @Get("modules")
    public Object modules(Mvc mvc) throws Exception {
        List<Module> modules = mvc.getModules();
        StringBuilder sb = new StringBuilder(2048);
        int i = 1;
