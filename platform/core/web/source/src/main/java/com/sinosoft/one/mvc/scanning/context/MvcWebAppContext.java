...
*/
package com.sinosoft.one.mvc.scanning.context;
...
public class MvcWebAppContext extends XmlWebApplicationContext {
    ...
    public MvcWebAppContext(ServletContext servletContext, String scope, boolean refresh) {
        this(servletContext, new LoadScope(scope, "controllers"), refresh);
    }

    public MvcWebAppContext(ServletContext servletContext, LoadScope scope, boolean refresh) {
        this.scope = scope;
    ...
    protected Resource[] getConfigResourcesThrows() throws IOException {
        return MvcResources.findContextResources(scope).toArray(new Resource[0]);
    }
    ...
    protected String[] getDefaultConfigLocations() {
        return new String[] { MvcWebAppContext.DEFAULT_CONFIG_LOCATION };
    }
    ...
    /**
     * Mvc对BeanFactory的特殊处理，必要时可以覆盖这个方法去掉Mvc的特有的处理
     * 
     * @throws IOException
     */
    protected void prepareBeanFactoryByMvc(ConfigurableListableBeanFactory beanFactory)
        ...
        if (messageBaseNames != null && messageBaseNames.length > 0) {
            if (logger.isDebugEnabled()) {
                logger.debug("[mvcWebApp/messages] starting registerMessageSourceIfNecessary");
            }
            registerMessageSourceIfNecessary(registry, messageBaseNames);
            if (logger.isDebugEnabled()) {
                logger.debug("[mvcWebApp/messages] finished registerMessageSourceIfNecessary");
            }
        }
    }
    ...
    protected String[] getMessageBaseNames() throws IOException {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("[mvcWebApp/messages] start  ...");
        }
        String[] messageBasenames = MvcResources.findMessageBasenames(scope);

        if (logger.isInfoEnabled()) {
            logger.info("[mvcWebApp/messages] exits ");
            logger.info("[mvcWebApp/messages] add default messages base name: "
                    + "'/WEB-INF/messages'");
        }
    ...
    /** 如果配置文件没有自定义的messageSource定义，则由Mvc根据最佳实践进行预设 */
