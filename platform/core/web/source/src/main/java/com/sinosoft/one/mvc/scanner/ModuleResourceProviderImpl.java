...
import static com.sinosoft.one.mvc.MvcConstants.CONF_INTERCEPTED_ALLOW;
import static com.sinosoft.one.mvc.MvcConstants.CONF_INTERCEPTED_DENY;
import static com.sinosoft.one.mvc.MvcConstants.CONF_MODULE_IGNORED;
import static com.sinosoft.one.mvc.MvcConstants.CONF_MODULE_PATH;
import static com.sinosoft.one.mvc.MvcConstants.CONF_PARENT_MODULE_PATH;
import static com.sinosoft.one.mvc.MvcConstants.CONTROLLERS;
...
import com.sinosoft.one.mvc.scanning.vfs.FileName;
import com.sinosoft.one.mvc.scanning.vfs.FileObject;
import com.sinosoft.one.mvc.scanning.vfs.FileSystemManager;
...
public class ModuleResourceProviderImpl implements ModuleResourceProvider {
    ...
    public List<ModuleResource> findModuleResources(LoadScope scope) throws IOException {
        ...
        List<ResourceRef> refers = MvcScanner.getInstance().getJarOrClassesFolderResources(
                controllersScope);
            ...
            if (!refer.hasModifier("controllers")) {
                if (logger.isDebugEnabled()) {
                    logger.debug("[moduleResource] Ignored because not marked as 'controllers'"
                            + " in META-INF/mvc.properties or META-INF/MANIFEST.MF: "
                            + resource.getURI());
                }
                continue;
            }
    ...
    private boolean isModuleResource(FileObject target) throws IOException {
        ...
        if (MvcStringUtil.matches(CONTROLLERS,name)) {
            //特定mvc的web包排除
            if(name.equals("web")&&target.getParent().getName().getBaseName().equals("mvc"))
                return false;
            else
                return true;
        }
    ...
    protected void checkModuleResourceCandidate(Local local, FileObject root,
        ...
        // 如果mvc.properties设置了controllers的module.path?
        FileObject mvcPropertiesFile = candidate.getChild("mvc.properties");
        if (mvcPropertiesFile != null && mvcPropertiesFile.exists()) {
            Properties p = new Properties();
            InputStream in = mvcPropertiesFile.getContent().getInputStream();
            p.load(in);
            in.close();
            ...
            if ("true".equalsIgnoreCase(ignored) || "1".equalsIgnoreCase(ignored)) {
                if (logger.isInfoEnabled()) {
                    logger.info("Ignored module(include submodules) by mvc.properties[ignored="
                            + ignored + "]: " + candidate);
                }
                return;
            }
            ...
            if (mappingPath != null) {
                ...
                mappingPath = MvcStringUtil.mappingPath(mappingPath);
                if(logger.isDebugEnabled()){
        ...
        moduleResource.setRelativePath(MvcStringUtil.relativePathToModulePath(relative));
    ...
    // FIXME: 如果一个module只有mvc.properties文件也会从moduleInfoList中remove，以后是否需要修改？
