 ...
 */
package com.sinosoft.one.mvc.web.annotation;
 ...
 * 使用，指示Mvc是否对所要分派的控制器(controller)及其方法(action)进行某些拦截器。
 * <p>
 ...
 * Mvc会自动把它们放到module中来，它们的id则为它们的类名(首字母改小写)或者去掉最后"Interceptor"后留下的名字(
 ...
 * Mvc优先找方法声明的Intercepted标注，然后再是类的标注。
 * <p>
 * 
 * <strong>扩展</strong>
 * <p>
 * 可以在每个module目录下的mvc.properties写intercepted.allow和intercepted.
 * deny属性配置该module的拦截器可见范围，使得该module下的控制器只能在这些可见的拦截下进行更进一步的选择。<br>
 * mvc.properties的这个特性不作用子module
 * 
 *
 * 
 */
